...
CSV.open("demo_log_total.csv", "wb") do |csv|
  csv << ["PAR ID","r ID","Region", "Provider", "DDPM", "OLD_HCBS_SCREENABLE_FLAG", "NEW_HCBS_SCREENABLE_FLAG"]

  for counter1 in 0...counter
     csv << ["#{par_info["PAR_ID"][counter1]}","#{par_info["r_ID"][counter1]}", par_info["REGION"][counter1], par_info["PROVIDER"][counter1], par_info["DDPM"][counter1], par_info["OLD_HCBS_SCREENABLE_FLAG"][counter1], par_info["NEW_HCBS_SCREENABLE_FLAG"][counter1] ]
  end
  #}
end
#=end
